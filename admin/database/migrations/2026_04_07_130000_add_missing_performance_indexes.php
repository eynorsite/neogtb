    ...
    public function up(): void
    {
        if (Schema::hasTable('posts')) {
            Schema::table('posts', function (Blueprint $table) {
                ...
                if (Schema::hasColumn('posts', 'deleted_at')) {
                    $table->index('deleted_at', 'posts_deleted_at_idx');
                }
            });
        }
        ...
        if (Schema::hasTable('site_pages')) {
            Schema::table('site_pages', function (Blueprint $table) {
                if (Schema::hasColumn('site_pages', 'slug')) {
                    $table->index('slug', 'site_pages_slug_idx');
                }
                if (Schema::hasColumn('site_pages', 'deleted_at')) {
                    $table->index('deleted_at', 'site_pages_deleted_at_idx');
                }
            });
        }
        ...
        if (Schema::hasTable('post_categories')) {
            Schema::table('post_categories', function (Blueprint $table) {
                if (Schema::hasColumn('post_categories', 'is_active')) {
                    $table->index('is_active', 'post_categories_is_active_idx');
                }
            });
        }
        ...
        if (Schema::hasTable('audit_leads')) {
            Schema::table('audit_leads', function (Blueprint $table) {
                if (Schema::hasColumn('audit_leads', 'email')) {
                    $table->index('email', 'audit_leads_email_idx');
                }
            });
        }
        ...
        if (Schema::hasTable('cee_leads')) {
            Schema::table('cee_leads', function (Blueprint $table) {
                if (Schema::hasColumn('cee_leads', 'email')) {
                    $table->index('email', 'cee_leads_email_idx');
                }
                if (Schema::hasColumn('cee_leads', 'sector')) {
                    $table->index('sector', 'cee_leads_sector_idx');
                }
            });
        }
        ...
        if (Schema::hasTable('navigation_items')) {
            Schema::table('navigation_items', function (Blueprint $table) {
                if (Schema::hasColumn('navigation_items', 'menu_id')) {
                    $table->index('menu_id', 'nav_items_menu_id_idx');
                }
                if (Schema::hasColumn('navigation_items', 'parent_id')) {
                    $table->index('parent_id', 'nav_items_parent_id_idx');
                }
            });
        }
        ...
        if (Schema::hasTable('admin_activity_logs')) {
            Schema::table('admin_activity_logs', function (Blueprint $table) {
                if (Schema::hasColumn('admin_activity_logs', 'admin_id')) {
                    $table->index('admin_id', 'admin_logs_admin_id_idx');
                }
                if (Schema::hasColumn('admin_activity_logs', 'created_at')) {
        ...
        if (Schema::hasTable('contact_messages')) {
            Schema::table('contact_messages', function (Blueprint $table) {
                if (Schema::hasColumn('contact_messages', 'deleted_at')) {
                    $table->index('deleted_at', 'contact_messages_deleted_at_idx');
                }
            });
        }
        ...
        if (Schema::hasTable('cookie_consents')) {
            Schema::table('cookie_consents', function (Blueprint $table) {
                if (Schema::hasColumn('cookie_consents', 'ip_hash')) {
                    $table->index('ip_hash', 'cookie_consents_ip_hash_idx');
                }
            });
        }
    }
