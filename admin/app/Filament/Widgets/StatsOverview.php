...
class StatsOverview extends BaseWidget
{
    ...
    {
        // ⚡ Bolt: Cache these expensive aggregation queries for 60 seconds
        // to prevent slow dashboard load times when tables grow large.

        $unread = Cache::remember('stats_overview_unread', 60, function () {
            return ContactMessage::where('status', 'new')->count();
        });

        $pendingGdpr = Cache::remember('stats_overview_pending_gdpr', 60, function () {
            return GdprRequest::where('status', 'pending')->count();
        });

        $overdueGdpr = Cache::remember('stats_overview_overdue_gdpr', 60, function () {
            return GdprRequest::where('status', 'pending')
                ->where('created_at', '<', now()->subDays(30))
                ->count();
        });

        $publishedPosts = Cache::remember('stats_overview_published_posts', 60, function () {
            return Post::where('status', 'published')->count();
        });

        $activePages = Cache::remember('stats_overview_active_pages', 60, function () {
