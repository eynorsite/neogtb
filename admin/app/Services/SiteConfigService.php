    ...
    public function jsonLd(string $type = 'Organization'): HtmlString
    {
        $jsonLd = Cache::remember('site_json_ld', self::CACHE_TTL, function () use ($type) {
            ...
            if (filled($s->company_address) || filled($s->company_city)) {
                ...
                if (filled($s->company_address)) {
                    $address['streetAddress'] = $s->company_address;
                }
                if (filled($s->company_city)) {
                    $address['addressLocality'] = $s->company_city;
                }
                if (filled($s->company_postal_code)) {
                    $address['postalCode'] = $s->company_postal_code;
                }
...
HTML;
    ...
    {
        $value = (string) Arr::get($this->settings()->legal_texts ?? [], $key, '');
