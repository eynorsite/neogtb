...
$exploreItems = [
  ...
  'conformer' => [
    ...
    'items' => [
      ...
      ['href' => '/modele-cctp-decret-bacs', 'title' => 'Modèle de CCTP', 'desc' => 'Trame de cahier des charges GTB classe B à adapter', 'icon' => 'document-text', 'match' => 'modele-cctp-decret-bacs'],
...
>
  <nav class="max-w-[1280px] 2xl:max-w-[1440px] mx-auto px-5 lg:px-10 flex items-center justify-between h-14 lg:h-[120px]">
    ...
    <div class="hidden lg:flex items-center gap-1">
      ...
      >
        {{ $site->label('nav.explorer', 'Explorer') }}
      ...
      <a href="/contact" class="text-[14px] font-medium transition-colors duration-200 px-4 py-2 rounded-lg min-h-[44px] flex items-center {{ request()->is('contact') ? 'text-accent-600 bg-accent-50' : 'text-dark-500 hover:text-dark-900 hover:bg-dark-50' }}" @if(request()->is('contact')) aria-current="page" @endif>{{ $site->label('nav.contact', 'Contact') }}</a>
    </div>

    {{-- CTA + search + Mobile toggle --}}
    <div class="flex items-center gap-2">
      <button type="button" aria-label="{{ $site->label('nav.search', 'Rechercher') }}" class="hidden lg:inline-flex items-center justify-center w-10 h-10 rounded-lg text-dark-500 hover:text-dark-900 hover:bg-dark-50 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500">
      ...
      @endif
      <button @click="openMobile()" class="lg:hidden p-2 rounded-lg hover:bg-dark-50 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center" aria-label="{{ $site->label('nav.open_menu', 'Ouvrir le menu') }}" aria-controls="mobile-drawer" :aria-expanded="mobileOpen.toString()">
  ...
  >
    ...
    <div class="max-w-[1280px] 2xl:max-w-[1440px] mx-auto px-6 md:px-10 py-2.5 flex items-center justify-between text-[11px] text-dark-500" style="border-top: 1px solid rgba(0,0,0,0.06);">
      <div class="flex items-center gap-2">
        <span>{{ $site->label('nav.help_prompt', "Besoin d'aide pour choisir ?") }}</span>
        <a href="/contact" class="text-accent-700 hover:text-accent-800 font-medium inline-flex items-center gap-1">
          {{ $site->label('nav.help_cta', 'Contactez un expert') }}
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto px-5 py-4">
        ...
        <div class="mt-2 flex flex-col">
          ...
          <a href="/contact" @click="closeMobile()" @if(request()->is('contact')) aria-current="page" @endif class="py-3 px-3 text-[15px] min-h-[44px] flex items-center rounded-xl {{ request()->is('contact') ? 'bg-accent-50 text-accent-700 font-semibold' : 'text-dark-900 font-medium hover:bg-dark-50' }}">{{ $site->label('nav.contact', 'Contact') }}</a>
        </div>
