            ...
            <div>
                <div class="rounded-2xl p-5 lg:p-7 bg-white border border-dark-100 lg:shadow-sm">
                    ...
                    <div class="p-4 rounded-lg bg-accent-50 border border-accent-200" role="alert">
                        <p class="text-sm font-medium text-accent-700">{{ $site->label('contact.success_title', 'Merci pour votre demande.') }}</p>
                        <p class="text-[13px] text-dark-500 mt-1">{{ $site->label('contact.success_message', 'Nous avons bien reçu votre message et reviendrons vers vous sous 48h ouvrées.') }}</p>
                    </div>
                    @else
                    <form
                        action="/contact/send"
                    ...
                    >
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                <label for="name" class="block text-[13px] font-medium text-dark-700 mb-1.5">{{ $site->label('forms.name', 'Nom') }}</label>
                                <input type="text" id="name" name="name" required placeholder="{{ $site->label('forms.name_placeholder', 'Votre nom') }}"
                            ...
                            <div>
                                <label for="email" class="block text-[13px] font-medium text-dark-700 mb-1.5">{{ $site->label('forms.email', 'Email professionnel') }}</label>
                                <input type="email" id="email" name="email" required placeholder="{{ $site->label('forms.email_placeholder', 'vous@entreprise.fr') }}"
                                    class="w-full px-4 py-2.5 text-[16px] lg:text-sm text-dark-900 bg-white rounded-lg border border-dark-200 outline-none transition-all focus:border-accent-500 focus:ring-2 focus:ring-accent-50 placeholder:text-dark-300" />
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" id="company" name="company" placeholder="{{ $site->label('forms.company_placeholder', 'Nom de votre entreprise') }}"
                                    class="w-full px-4 py-2.5 text-[16px] lg:text-sm text-dark-900 bg-white rounded-lg border border-dark-200 outline-none transition-all focus:border-accent-500 focus:ring-2 focus:ring-accent-50 placeholder:text-dark-300" />
                            </div>

                            <div>
                                <label for="subject" class="block text-[13px] font-medium text-dark-700 mb-1.5">{{ $site->label('forms.subject', 'Sujet') }}</label>
                                <select id="subject" name="subject" required
                                    ...
                                    <option value="" disabled selected>{{ $site->label('forms.subject_placeholder', 'Choisir un sujet') }}</option>
                                    <option value="Avis sur un devis">{{ $site->label('forms.subject_quote', 'Avis sur un devis') }}</option>
                                    <option value="Aide au choix technologique">{{ $site->label('forms.subject_tech', 'Aide au choix technologique') }}</option>
                                    <option value="Question réglementaire">{{ $site->label('forms.subject_regulation', 'Question réglementaire') }}</option>
                                    <option value="Demande d'audit">{{ $site->label('forms.subject_audit', "Demande d'audit") }}</option>
                                    <option value="Autre">{{ $site->label('forms.subject_other', 'Autre') }}</option>
                                </select>
                            </div>

                            <div>
                                <label for="message" class="block text-[13px] font-medium text-dark-700 mb-1.5">{{ $site->label('forms.message', 'Message') }}</label>
                                <textarea id="message" name="message" rows="4" required placeholder="{{ $site->label('forms.message_placeholder', 'Décrivez votre contexte...') }}"
                            ...
                            <div>
                                <button type="submit" :disabled="sending"
                                    ...
                                    <span x-show="!sending">{{ $site->label('forms.submit', 'Envoyer ma demande') }}</span>
                                    <span x-show="sending" class="flex items-center gap-2">
                                        <svg class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" />
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                                        </svg>
                                        {{ $site->label('forms.sending', 'Envoi en cours...') }}
                                    </span>
                                </button>
                            </div>
