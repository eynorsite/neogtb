...
<section class="{{ $hauteur }} relative flex items-center justify-center overflow-hidden bg-gradient-to-br from-dark-950 via-primary-900 to-dark-900">
    {{-- Floating decorative circles (like Astro) --}}
    ...
    <div class="relative z-10 mx-auto max-w-4xl px-4 py-24 {{ $align }}">
        @if(!empty($content['titre']))
            <h1 class="text-4xl font-heading font-extrabold leading-tight text-white sm:text-5xl lg:text-6xl animate-fade-in-up">
                {!! preg_replace('/\b(GTB|GTC|NeoGTB)\b/', '<span class="text-gradient">$1</span>', e($content['titre'])) !!}
            </h1>
        @endif

        @if(!empty($content['sous_titre']))
            <p class="mt-6 text-lg text-dark-300 sm:text-xl lg:text-2xl animate-fade-in-up" style="animation-delay: 0.15s;">
                {{ $content['sous_titre'] }}
            </p>
        @endif
        ...
        @if(!empty($content['cta_texte']))
            <div class="mt-10 flex flex-wrap gap-4 {{ $settings['alignement'] ?? 'center' === 'center' ? 'justify-center' : '' }} animate-fade-in-up" style="animation-delay: 0.45s;">
