            ...
            @foreach($content['questions'] ?? [] as $i => $q)
                <div class="overflow-hidden transition-all duration-300 animate-fade-in-up"
                    ...
                    <button @click="open = open === {{ $i }} ? null : {{ $i }}"
                            ...
                            class="flex w-full cursor-pointer items-center justify-between py-4 px-2 -mx-2 text-left transition-colors duration-200 hover:text-accent-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 rounded-lg">
