<section class="py-16 lg:py-24">
    <div class="mx-auto max-w-3xl px-4 sm:px-6">
        @if(!empty($content['titre']))
            <h2 class="mb-8 text-2xl font-heading font-extrabold text-dark-900">{{ $content['titre'] }}</h2>
        @endif

        @if(session('contact_success'))
            <div class="mb-8 rounded-xl bg-accent-50 border border-accent-200 p-6 text-center">
                <span class="text-2xl">✅</span>
                <p class="mt-2 font-semibold text-accent-700">{{ $content['message_succes'] ?? 'Message envoyé !' }}</p>
            </div>
        @endif
