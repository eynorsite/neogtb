...
<section class="py-16 lg:py-32 text-center">
    <div class="max-w-[600px] mx-auto px-5 lg:px-6">
        ...
        <h1 class="font-heading text-[28px] font-medium text-dark-900 tracking-tight mb-3">{{ $site->label('newsletter.confirmed.title', 'Inscription confirmée') }}</h1>
        <p class="text-base text-dark-500 leading-relaxed mb-8">
            {{ $site->label('newsletter.confirmed.text', 'Vous recevrez la veille GTB mensuelle NeoGTB dans votre boîte mail. 1 email par mois, désabonnement en 1 clic.') }}
        </p>
        <a href="/" class="inline-flex items-center gap-2 px-6 py-3 bg-primary-600 text-white text-sm font-semibold rounded-lg hover:bg-primary-700 transition-colors btn-glow">
            {{ $site->label('newsletter.confirmed.cta', 'Retour à l\'accueil') }}
