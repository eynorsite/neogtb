    ...
    <div class="max-w-[720px] mx-auto px-5 lg:px-10">
        ...
        <div class="mb-12">
            ...
            <div x-data="{ open: false }" class="border-t border-dark-200 @if($loop->last) border-b @endif">
                <button type="button" @click="open = !open" :aria-expanded="open ? 'true' : 'false'" aria-controls="faq-panel-{{ $loop->parent->index ?? 0 }}-{{ $index }}" class="w-full flex items-center justify-between py-5 min-h-[56px] text-left bg-transparent border-none cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500 focus-visible:ring-offset-2 rounded-lg">
                ...
                <div id="faq-panel-{{ $loop->parent->index ?? 0 }}-{{ $index }}" x-show="open" x-collapse>
                    <p class="text-sm text-dark-500 leading-relaxed pb-5 px-1">{!! $item['answer'] ?? '' !!}</p>
                </div>
            </div>
            @endforeach
        </div>
