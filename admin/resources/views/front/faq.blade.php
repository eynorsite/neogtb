        ...
        <div class="mb-12">
            ...
            <div x-data="{ open: false }" class="border-t border-dark-200 @if($loop->last) border-b @endif">
                <button @click="open = !open"
                        ...
                        aria-controls="faq-panel-{{ $loop->parent->iteration }}-{{ $index }}"
                        class="w-full flex items-center justify-between py-5 min-h-[56px] text-left bg-transparent border-none cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500 rounded-lg">
                ...
                <div id="faq-panel-{{ $loop->parent->iteration }}-{{ $index }}" x-show="open" x-collapse>
