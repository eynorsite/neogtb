...
@verbatim
{
    ...
    "mainEntity": [
        ...
        {"@type":"Question","name":"Mon bâtiment est-il concerné par le décret BACS ?","acceptedAnswer":{"@type":"Answer","text":"Si votre bâtiment est tertiaire et que sa puissance CVC dépasse 290 kW, vous devez disposer d'un système BACS depuis le 1er janvier 2025 (au 1er janvier 2030 pour les bâtiments entre 70 et 290 kW). Le décret impose des fonctions d'automatisation (art. R. 175-3), pas une classe ; la classe B de la norme NF EN ISO 52120-1 est le niveau couramment visé et la condition de la prime CEE."}},
        {"@type":"Question","name":"Que signifient les classes A, B, C, D de la norme ISO 52120-1 (ex-EN 15232) ?","acceptedAnswer":{"@type":"Answer","text":"Classe D : aucune automatisation. Classe C : automatisation standard, conforme au décret BACS. Classe B : automatisation avancée avec supervision centralisée, niveau couramment visé et condition de la prime CEE. Classe A : haute performance avec optimisation énergétique et gestion prédictive."}},
    ...
    <div class="max-w-[720px] mx-auto px-5 lg:px-10">
        ...
        <div class="mb-12">
            ...
            <div x-data="{ open: false }" class="border-t border-dark-200 @if($loop->last) border-b @endif">
                <button @click="open = !open" :aria-expanded="open ? 'true' : 'false'" aria-controls="faq-item-{{ $loop->parent->index }}-{{ $index }}" class="w-full flex items-center justify-between py-5 min-h-[56px] text-left bg-transparent border-none cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500 focus-visible:ring-offset-2 rounded-lg">
                    <span class="text-[15px] font-medium text-dark-900 pr-4">{{ $item['question'] ?? '' }}</span>
                    <svg :class="open && 'rotate-180'" class="w-4 h-4 text-dark-500 transition-transform duration-200 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                </button>
                <div id="faq-item-{{ $loop->parent->index }}-{{ $index }}" x-show="open" x-collapse>
