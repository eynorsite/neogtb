        ...
        <div class="mb-12">
            ...
            <div x-data="{ open: false }" class="border-t border-dark-200 @if($loop->last) border-b @endif">
                <button @click="open = !open"
                        ...
                        class="w-full flex items-center justify-between py-5 min-h-[56px] text-left bg-transparent border-none cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-500 rounded-lg px-2 -mx-2">
                ...
                <div x-show="open" x-collapse id="faq-answer-{{ $index }}" aria-labelledby="faq-question-{{ $index }}">
                    <p class="text-sm text-dark-500 leading-relaxed pb-5 px-2 -mx-2">{!! $item['answer'] ?? '' !!}</p>
                </div>
            </div>
            @endforeach
        </div>
